...
from .base import ThemePlugin, PluginInfo, PluginError, PluginCompatibilityError, PluginInitializationError
...
class PluginManager:
            ...
            for module_info in pkgutil.iter_modules([str(plugin_path)]):
                ...
                try:
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if (issubclass(obj, ThemePlugin) and 
                            obj is not ThemePlugin and 
    ...
    def register_plugin(self, plugin_class: Type[ThemePlugin]) -> None:
        ...
        """
        if not (inspect.isclass(plugin_class) and 
                issubclass(plugin_class, ThemePlugin) and 
                plugin_class is not ThemePlugin):
    ...
    def check_plugin_compatibility(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to check compatibility for plugin {plugin_name}: {e}")
            return False
